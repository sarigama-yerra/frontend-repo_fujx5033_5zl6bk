    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-900/70 backdrop-blur border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm">
          ...
          <button onClick={onOpenCart} className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-md">
      ...
      {open && (
        <div className="md:hidden border-t border-white/10 bg-slate-900/90">
          <div className="px-4 py-3 space-y-2">
            ...
            <button onClick={() => { setOpen(false); onOpenCart && onOpenCart(); }} className="block w-full text-left text-white bg-blue-600 px-3 py-2 rounded">Open Cart</button>
          </div>
        </div>
      )}
    </header>
